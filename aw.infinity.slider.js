    ...
    $.fn.extend({
        slider: function (param){
            this.each(function (){
                ...
                if(!param ||(param && param.screenwidth==true)){
                ...
                if(param && param.auto==true){//设置自动播放
                    time=param.time?param.time:10000;//默认时间间隔10000ms
                    if(timer){
                        clearInterval(timer);
                    }
                        ...
                        else
                        {
                            ...
                            if(left<0)
                            {
                                if(isStandardBrowser){//标准浏览器
                                    cssData = 'translateX('+Math.round(left%W)+'px)';
                                    cssTxt = 'width:'+oUlWidth+';'+ setTransform(cssData);
                                    obj.style.cssText=cssTxt;
                                }else{
                                    obj.style.left=Math.round(left%W)+'px';
                            ...
                            else
                            {
                                ...
                                if(isStandardBrowser){//标准浏览器
                                    cssData = 'translateX('+Math.round((left%W-W)%W)+'px)';
                                    cssTxt = 'width:'+oUlWidth+';'+ setTransform(cssData);
                                    obj.style.cssText=cssTxt;
                                }else{
                                    obj.style.left=Math.round((left%W-W)%W)+'px';
                                    obj.style.width = oUlWidth;
                                }
